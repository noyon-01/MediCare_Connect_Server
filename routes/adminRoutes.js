...
router.patch('/doctors/:id/verify', async (req, res) => {
  try {
    ...
    const doctorProfile = await collections.doctors.findOne({ _id: new ObjectId(doctorIdStr) });
    ...
    if (verificationStatus !== 'verified') {
      const activeAppointments = await collections.appointments.find({
...
router.get('/analytics', async (req, res) => {
  try {
    const totalDoctors = await collections.doctors.countDocuments({ verificationStatus: 'verified' });
    const totalPatients = await collections.users.countDocuments({ role: 'patient' });
    ...
    const docs = await collections.doctors.find({ verificationStatus: 'verified' }).toArray();
    ...
    const allDoctorsList = await collections.doctors.find({ verificationStatus: 'verified' }).toArray();
    ...
    const allAppointmentsForDistinct = await collections.appointments.find({}, { projection: { patientId: 1 } }).toArray();
